...
const mockGetMediaInfo = vi.fn<
  () => import('../whatsapp-media.js').MediaInfo | null
>(() => null);
const mockDownloadAndSaveMedia =
  vi.fn<() => Promise<import('../whatsapp-media.js').MediaResult | null>>();
const mockUnwrapMessage = vi.fn((msg: unknown) => msg);

vi.mock('../whatsapp-media.js', () => ({
  getMediaInfo: (...args: unknown[]) =>
    mockGetMediaInfo(...(args as Parameters<typeof mockGetMediaInfo>)),
  downloadAndSaveMedia: (...args: unknown[]) =>
    mockDownloadAndSaveMedia(
      ...(args as Parameters<typeof mockDownloadAndSaveMedia>),
    ),
  unwrapMessage: (...args: unknown[]) =>
    mockUnwrapMessage(...(args as Parameters<typeof mockUnwrapMessage>)),
}));
...
describe('WhatsAppChannel', () => {
  beforeEach(() => {
    ...
    mockUnwrapMessage.mockReset().mockImplementation((msg: unknown) => msg);
  });
